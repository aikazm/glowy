    ...
    <b-container>
      <b-row>
        <b-col v-for="product in products" :key="product.id">
          <b-card
            :title="product.name"
            :img-src="require(`@/assets/nivea-${product.id}.jpg`)"
            img-top
            tag="article"
          ...
          >
            ...
            <b-burron
              ...
              href="#"
              variant="warning"
            >Product added to Shopping Cart</b-burron>
          </b-card>
        </b-col>
      </b-row>
      ...
      <b-row>
        <b-col>
          <b-table bordered hover:items="cart" :fields="fields">
            <template slot="#" slote-scope="data">{{data.index+1}}</template>
            <template slot="price" slot-scope="data">
              <b-button @click="remove(data.item.id)" vartiant="danger" class="mr-2">X</b-button>
            </template>
            ...
            <template slot="image" slot-scope="data">
              <b-img
                ...
                :src="require(`@/assets/nivea-${data.item.id}.jpg`)"
                fluid
              />
            </template>
          </b-table>
...
export default {
  ...
  methods: {
    add(product) {
      this.products[product.id - 1].cart = truethis.cart.push(product);
      this.counter++;
    },
    clean() {
      this.cart = [];
